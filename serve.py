...
def FLAG():
    flag = valid_url_prefixs[request.user_prefix] + session['genesis_block_hash'][4:8] + request.user_prefix[5:8] + \
           session['genesis_block_hash'][12:16]
    try:
...
if os.path.isfile(KEY_FILENAME):
    ...
    obj = {'bank': [bank_address, bank_privkey], 'hacker': [hacker_address, hacker_privkey],
           'shop': [shop_address, shop_privkey], 'cold_wallet': [shop_wallet_address, shop_wallet_privkey]}
...
def create_tx(input_utxo_ids, output_utxo, privkey_from=None):
    tx = {'input': input_utxo_ids, 'signature': [sign_input_utxo(id, privkey_from) for id in input_utxo_ids],
          'output': output_utxo}
    tx['hash'] = hash_tx(tx)
    return tx
